...
export class BoardUI {
  constructor({ boardEl, arrowSvg, promoEl, evalbar, onUserMove, getPieceAt, getLegalTargets }){
    ...
    this.userArrows = new Set(); // 'e2e4' or colored as 'e2e4:g'
    this.userCircles = new Set(); // 'e4:g'
...
finishRightDrag(e){
    ...
    // Short click or tiny drag → toggle circle on source square
    if (!movedEnough || toSq === start.fromSq){
      ...
      if (this.userCircles.has(key)){
        ...
        if (this.gMarks) this.drawCircleOnLayer(this.gMarks, start.fromSq, color, sw, key);
      }
      return;
    }

    // Real drag → toggle arrow with color
    ...
    if (this.userArrows.has(akey)){
      ...
      if (this.gUser) this.drawArrowOnLayer(this.gUser, start.fromSq, toSq, color, sw, akey);
    }
  }
 else {
  ...
  finishRightDrag(e){
    ...
    const toSq = this._coordsToSquare(e);
    // Threshold to avoid accidental clicks
    const dx = e.clientX - start.x0, dy = e.clientY - start.y0;
    const movedEnough = (dx*dx + dy*dy) > 16; // 4px

    if (!toSq || !movedEnough || toSq === start.fromSq) return;

    const uci = start.fromSq + toSq;
    if (this.userArrows.has(uci)){
      // toggle off
      this.userArrows.delete(uci);
      const node = this.gUser.querySelector(`[data-uci="${uci}"]`);
      node && node.remove();
    } else {
      // add and draw
  ...
  setUserDrawings({ arrows=[], circles=[] } = {}){
    ...
    for (const a of arrows){ const key = `${a.uci}:${a.color||'g'}`; this.userArrows.add(key); }
    for (const c of circles){ const key = `${c.sq}:${c.color||'g'}`; this.userCircles.add(key); }
...
// ===== Public API =====
  ...
  setOrientation(side){
    ...
    this.renderPosition();
    this.redrawUserDrawings?.(); // keep drawings aligned after flip
  }
